...
int net_tun_device_init_dev(net_tun_driver_t driver, net_tun_device_t device, const char * name, void * tunnelFlow) {
    if (tunnelFlow == NULL) {
        CPE_ERROR(driver->m_em, "%s: init dev: no tunnelFlow", name);
        return -1;
    }
    
    device->m_tunnelFlow = (NEPacketTunnelFlow *)tunnelFlow;
