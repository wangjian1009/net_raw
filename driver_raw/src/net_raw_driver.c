...
static int net_raw_driver_init(net_driver_t base_driver) {
    ...
    TAILQ_INIT(&driver->m_free_device_listeners);    
...
static void net_raw_driver_fini(net_driver_t base_driver) {
    ...
    while(!TAILQ_EMPTY(&driver->m_free_device_listeners)) {
        net_raw_device_listener_real_free(TAILQ_FIRST(&driver->m_free_device_listeners));
    }
