...
net_raw_device_tun_create(net_raw_driver_t driver, const char * name) {
    ...
    if (sock < 0) {
        CPE_ERROR(driver->m_em, "raw: device %s: socket fail, %d %s", device_tun->m_dev_name, errno, strerror(errno));
        goto create_error;
    }
    ...
    if (ioctl(sock, SIOCGIFMTU, (void *)&ifr) < 0) {
        CPE_ERROR(driver->m_em, "raw: device %s: get mtu fail, %d %s", device_tun->m_dev_name, errno, strerror(errno));
        close(sock);
        goto create_error;
    }
    ...
    if (ioctl(sock, SIOCGIFADDR, (void *)&ifr) < 0) {
        CPE_ERROR(driver->m_em, "raw: device %s: get addr fail, %d %s", device_tun->m_dev_name, errno, strerror(errno));
        close(sock);
        goto create_error;
    }
    ...
    if (ioctl(sock, SIOCGIFNETMASK, (void *)&ifr) < 0) {
        CPE_ERROR(driver->m_em, "raw: device %s: get mask fail, %d %s", device_tun->m_dev_name, errno, strerror(errno));
        close(sock);
        goto create_error;
    }
    ...
    if (driver->m_debug > 0) {
        ...
        CPE_INFO(
            driver->m_em, "raw: device %s: created: mtu=%d, address=%s, mask=%s",
